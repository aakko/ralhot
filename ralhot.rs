...
#![deny(missing_docs)] // does not work for macros D:
...
pub trait OptionParser {
    ...
    type Parsed;
    /// The error string will be prepended with the program's name and
    /// the option and argument that caused the error.
    fn parse_option_param(&str) -> Result<Self::Parsed, String>;
}

impl OptionParser for char {
    type Parsed = char;

    fn parse_option_param(s: &str) -> Result<char, String> {
        let mut c = s.chars();
        let a = c.next();
        let b = c.next();
        match (a, b) {
...
macro_rules! impl_option_parser {
    ($($typ:ident),*) => {$(
        impl OptionParser for $typ {
            type Parsed = $typ;

            fn parse_option_param(s: &str) -> Result<$typ, String> {
                use std::str::FromStr;
...
/// Internal stuff that needs to be public so that macros can use it.
#[doc(hidden)]
pub mod internal {
    use std;
    ...
    pub trait OptionParser {
        ...
        fn parse_option(parse: &Parse, i: usize)
                        -> Result<Option<Self::Parsed>, ParseError> {
            match parse.option[i] {
                None => Ok(None),
                Some(_) => Self::parse_required(parse, i).map(|v| Some(v))
            }
        }

        fn parse_required(parse: &Parse, i: usize)
                          -> Result<Self::Parsed, ParseError>;
    }

    impl<T: super::OptionParser> OptionParser for T {
        type Parsed = T::Parsed;

        fn parse_required(parse: &Parse, i: usize)
                          -> Result<Self::Parsed, ParseError> {
            let param = parse.option[i].as_ref().unwrap();
            T::parse_option_param(param) .map_err(|e| {
                Fail(format!("{}: {}: {}\n", parse.program,
                             parse.source[i].as_ref().unwrap(), e))
            })
        }
    }
    ...
    macro_rules! ralhot_type {
        ...
        (flag) => { bool };
        (req $typ:ident) => {
            <$typ as $crate::internal::OptionParser>::Parsed
        };
        ($typ:ident) => {
            Option<<$typ as $crate::internal::OptionParser>::Parsed>
        };
    }
    ...
    macro_rules! ralhot_unwrap {
        ...
        ($parse:ident [req $typ:ident] $name:ident) => {{
            use $crate::internal::OptionParser;
            $typ::parse_required(&$parse, $name)?
        }};
        ($parse:ident [$typ:ident] $name:ident) => {{
            use $crate::internal::OptionParser;
            $typ::parse_option(&$parse, $name)?
        }};
    }
         ...
         [$([[$($typ:tt)*] [$name:ident] $($alias:expr),*])*]] => {
            ...
            impl $class {
                fn _parse() -> Self {
                    ...
                    match Self::parse_noexit(std::env::args()) {
                        Ok(parse) => return parse,
                        Err(ParseError::Fail(msg)) => {
                            write!(std::io::stderr(), "{}", msg).unwrap();
                            std::process::exit(1);
                        },
                        Err(ParseError::Succeed(msg)) => {
                ...
                fn parse_noexit<T: IntoIterator<Item=String>>(args: T)
                                -> Result<Self, $crate::internal::ParseError> {
                    use $crate::internal::*;
                    let mut _parser = Parser::new();
                    let mut _i = 0;
                    $(let $name = _i;
                      _parser
                      .add_option(Opt::new(ralhot_required!($($typ)*),
                                           ralhot_flag!($($typ)*),
                                           &[stringify!($name)
                                             $(, $alias)*]));
                      _i += 1;)*
                        $(ralhot_attr!(_parser, $k, $v);)*
                        let _parse = _parser.parse(args)?;
    ...
    type Set<A> = std::collections::HashSet<A>;
    ...
    fn spaces(spaces: usize) -> impl Iterator<Item=char>
    {
        std::iter::repeat(' ').take(spaces)
    }
    ...
    fn wrap_description(mut column: usize,
        ...
        if column < indent - 2 {
            s.extend(spaces(indent - 2 - column));
            column = indent - 2;
        }
        let indent = indent + 2;
            ...
            for &alias in aliases {
                ...
                match alias.chars().next() {
                    ...
                    _ => {
                        ...
                        if eq.is_some() {
                            if argument.is_some() {
                                panic!("ralhot: multiple arg names: '{}'",
                                       alias);
                            }
                            argument = Some(alias[(eq.unwrap() + 1)..]
                                            .to_owned());
                        }
                        let alias = match eq {
                            None => alias,
    ...
    impl Parser {
      ...
      --version  output version information and exit\n";
        ...
        pub fn parse<'a, T>(&mut self, args: T) -> Result<Parse, ParseError>
            ...
            if !self.attribute.contains_key("program") {
                ...
                let program = argv0.unwrap().split('/').last()
                    .unwrap().to_owned();
            ...
            let mut parse = Parse {
                ...
                leftovers: Vec::with_capacity(args.size_hint().0),
            ...
            while let Some(arg) = args.next() {
                if self.stopwords.contains(&arg) {
                    ...
                    parse.leftovers.push(arg);
                    parse.leftovers.extend(args);
                    break;
                }
                ...
                match (a, b) {
                    (Some('-'), Some('-')) => {
                        ...
                        if self.option[i].argument.is_none() {
                            ...
                            if let Some(byte) = eq {
                                let v = &arg[(byte + 1)..];
                                parse.option[i] = Some(v.to_owned());
                                parse.source[i] = Some(format!("--{}={}",
                                                               opt, v));
                            } else {
                                match args.next() {
                                    Some(v) => {
                    ...
                    (Some('-'), Some(_)) => {
                        ...
                        while let Some(opt) = chars.next() {
                            ...
                            if chars.as_str() != "" {
                                let param: String = chars.collect();
                                parse.source[i] =
                                    Some(format!("-{}{}", opt, &param));
                                parse.option[i] = Some(param);
                                break;
                            } else {
                                match args.next() {
                                    Some(v) => {
